...
// 다차원 배열에서 배열을 매개변수로 받을 경우 배열의 첫번째 주소(포인터)를 받기 때문에 포인터 연산의 단위를 함수가 알기위해서 cols는 생략할 수 없지만 rows는 생략할 수 있다.
#include <stdio.h>
...
int sum2d_3(int* ar, int row, int col)  // 포인터를 이용할 경우 다차원을 1차원 배열로 사용할 수 있다.
{
    int r, c, tot = 0;
    for (r = 0; r < ROWS; r++)
        for (c = 0; c < COLS; c++)
