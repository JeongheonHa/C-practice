...
// a_ptr : a변수의 주소값를 저장하는 변수
// *a_ptr : a_ptr변수가 가리키는 값을 사용하고자 할때 사용
// * : 간접접근 연산자 (indirection operator)
// ex) int *a_ptr = &a;
...
int main()
{
    int a, b;   // 변수 이름 a, b를 통해서 메모리 공간을 사용한다. 변수 이름이 내부적으로 주소를 알고있고 메모리 공간의 크기는 int가 결정
